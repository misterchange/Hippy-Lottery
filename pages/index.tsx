...
const Home: NextPage = () => {
  ...
  const handleClick =async () => {
    ...
    try {
      ...
      console.info("Contract call sucsess!",data);
    } catch (err) {
  ...
  if(isLoading) return (
    <div className='bg-[#091B18] h-screen flex flex-col items-center justify-center '>
      <div className='flex items-center space-x-5 mb-10'>
        <img className='rounded-full h-20 w-20' src="https://i.postimg.cc/63qXyM7w/HIPPY-1.png" alt="" />
        <h1 className='text-lg text-white font-bold '>Loading HIPPY LOTTERY</h1>
      </div>
  ...
  return (
    <div className="bg-[#091B18] min-h-screen flex flex-col">
      <Head>
        <title>Hippy Draw</title>
      </Head>
      <Header /> 
      <Marquee className='bg-[#0A1F1C] p-5 mb-5' gradient={false} speed={120}>
    ...
    {winnings > 0 && (
      <div className="max-w-md md:max-w-2xl lg:max-w-4xl mx-auto mt-5">
        <button onClick={onWithdrawWinnings} className=" p-5 bg-gradient-to-b from-orange-500 to-emerald-600 animate-pulse text-center rounded-xl w-full" >
          <p className='font-bold'>Congragulations!!!(Winner Winner Chicken Dinner!!!) you hav won the Lottery</p>
          <p className='mt-3'>Total Winnings :{ethers.utils.formatEther(winnings.toString())}{" "}{currency}</p>
          <br/>
      ...
      <div className='space-y-5 md:space-y-0 m-5 md:flex md:flex-row items-start justify-center md:space-x-5'>
        ...
        <div className='stats-container'>
          ...
          <div className='flex text-white items-center space-x-2 bg-[#091B18] border-[#004337] border p-4 rounded-md'>
            <p>TICKETS</p>
            <input className='flex w-full bg-transparent text-center outline-none' 
            type="number" 
            min={1}
            max={10}
            value={quantity}
          ...
          '>Buy {quantity} ticket(s) for {ticketPrice && Number(ethers.utils.formatEther(ticketPrice.toString()))*quantity}{" "}{currency} </button>
          </div>
            ...
            <div className='stats'>
                ...
                <div className='flex max-w-sm flex-wrap gap-x-2 gap-y-2'>
                  {Array(userTickets).fill("").map((_,index) =>(
                    <p key={index} 
                    className="text-emerald-300 h-16 w-10 bg-emerald-500/30 rounded-lg flex flex-shrink-0 items-center justify-center text-xs italic">{index +1}</p>
                  ))}
                </div>
            </div>
          ) }
